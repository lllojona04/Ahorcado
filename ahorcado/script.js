document.addEventListener('DOMContentLoaded', () => {
  ...
  const instruccionesDiv = document.getElementById('instruccionesDiv');
  const gameContainer = document.querySelector('.container');

  // Habilita el botón 'Comenzar Juego' solo cuando el checkbox está marcado
  confirmCheckbox.addEventListener('change', () => {
    startGameBtn.disabled = !confirmCheckbox.checked;
  });

  // Maneja el clic en el botón 'Comenzar Juego'
  startGameBtn.addEventListener('click', () => {
    if (confirmCheckbox.checked) {
      instruccionesDiv.style.display = 'none'; // Oculta instrucciones
      gameContainer.style.display = 'block'; // Muestra juego principal
    }
  });

  // Muestra las instrucciones al cargar la página
  instruccionesDiv.style.display = 'flex';
...
$(document).ready(function() {
  ...
  const $timerSpan = $('#timer'); // ahora coincide con <span id="timer">
  ...
  function updateTimerColor() {
    if (timeLeft > 30) {
      $timerSpan.css('color', 'green');
    } else if (timeLeft > 10) {
      $timerSpan.css('color', 'orange');
    } else {
